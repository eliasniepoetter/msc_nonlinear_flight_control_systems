...
\todo{include defintion of volume}
...
\todo{add $\forall j [1 ... 2m+1]$ and selecet the $j$ for corresponding constraint}
...
Note that the $\sin()$ term is approximated by a Taylor series epansion of degree five in order to obtain a polynomial system. \todo{answer Renatos question regarding limits of Taylor approx.} The constant $k$ combines 
...
\todo{briefly describe implementation setup (casadi etc.)}


\subsection{Results}
\todo{add results of terminal condition synthesis}
...
stable around the origin. This approach is therefore not suitbale for systems with periodic limit cycles. \todo{add requirement to section \ref{sec:synTermCond}} It can further be quite difficult to obtain a 
