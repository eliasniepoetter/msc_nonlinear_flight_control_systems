function [u, flag] = VdP_OCP_terminal(x1_current, x2_current,P,alpha)
...
%terminalCosts = (X1(N+1) + X2(N+1))*wT;
opti.minimize(terminalCosts + wx1*(X1*X1') + wx2*(X2*X2') + wu*(U*U'));
...
opti.subject_to(X2>-10);                        %
opti.subject_to(U<90);                          % control constraints
opti.subject_to(U>-90);                         %
